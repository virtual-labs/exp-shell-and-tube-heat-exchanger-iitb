...
function activity6() {
	...
	control_panel_text = new Chemistry.Text(
		'Control Panel',
		new Chemistry.Point(850, 790),
		canvas
	);

	control_panel_text.font = '23vw Arial';

	let v1 = new Chemistry.Custome_image(
		blue_valve,
		new Chemistry.Point(400, 410),
		66,
		19,
		canvas
	);
	v1.stang = -90;
	let v2 = new Chemistry.Custome_image(
		blue_valve,
		new Chemistry.Point(605, 520),
		66,
		19,
		canvas
	);
...
function draw_seq_all() {
	...
	if (seq == 0) {
		...
		pp.showdescription(
			`<p class='discription_text'>open cold water inlet valve</p>`,
			3
		);
		// show_right_pannel();
	}
	...
	if (seq == 1 && seq_container[1].l < seq_container[1].l_last) {
		...
		if ((seq_container[1].name = 'second')) {
			...
			pp.showdescription(
				`<p class='discription_text'>Open glass section outlet valve</p>`,
				3
			);
			show_right_pannel(3);
		}
		...
		if ((seq_container[1].name = 'third')) {
			...
			var second_geo = new Chemistry.anim_image_x_dir(
				seq3_img,
				new Chemistry.Point(470, 440),
				709,
				650,
		...
		if ((seq_container[1].name = 'fourth')) {
			...
			var second_geo1 = new Chemistry.anim_image_y_dir_down(
				seq4_img,
				new Chemistry.Point(470, 440),
				709,
				650,
		...
		if ((seq_container[1].name = 'fourth')) {
			...
			pp.showdescription(
				`<p class='discription_text'>Click on the 'h' button on control pannel to turn on the Heater</p>`,
				3
			);
			// second_geo.draw();
		...
		pp.showdescription(
			`<p class='discription_text'>Open outlet glass valve</p>`,
			3
		);
		show_right_pannel(3);
		...
		if ((seq_container[1].name = 'fifth')) {
			...
			pp.showdescription(
				`<p class='discription_text'>Note down all the readings, you'll require it to fill the table in the next activity</p>`,
				3
			);
			show_right_pannel(3);
			show_table_0 = true;
			...
			if (!document.getElementById('table-btn')) {
				pp.addtorightpannel(
					`<button style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;" id="table-btn" class="btn btn-primary" onclick="load_observation_table();">Next</button>`,
					3
				);
			}
		}
	...
	if (y >= 494 && y <= 545) {
		if (x >= 598 && x <= 681) {
			...
			let slider: HTMLInputElement = <HTMLInputElement>(
				document.getElementById('level-slider')
			);
			slider.value = '1';
			...
			pp.showdescription(
				`<p class='discription_text'>Click on 'p' button on control panel to turn on the pump</p>`,
				3
			);
...
function a6_mouseclick_seq_35(e: MouseEvent) {
	pp.showdescription(
		`<p class='discription_text'>Please wait for the animation to complete</p>`,
		3
	);
	show_right_pannel(3);
	let x = Math.round((e.clientX - rect.x) / lscale);
...
function a6_mouseclick_seq_7(e: MouseEvent) {
	...
	if (y >= 688 && y <= 769) {
		if (x >= 943 && x <= 1020) {
			...
			console.log(
				'click on up arrow to select desire temperature and click start on timer'
			);

			//timer image
			var second_geo = new Chemistry.anim_image(
				seq5_img,
				new Chemistry.Point(1000, 325),
				290,
				210,
				canvas
			);
			...
			pp.showdescription(
				`<p class='discription_text'>Click on up and down arrow on temp controller to select the temperature</p>`,
				3
			);
			show_right_pannel(3);
		}
...
function a6_mouseclick_seq_8(e: MouseEvent) {
	...
	if (y >= 408 && y <= 440) {
		if (x >= 700 && x <= 780) {
			...
			var second_geo = new Chemistry.Geo_Text(
				temp_con_value.toString(),
				new Chemistry.Point(670, 390),
				canvas
			);
			...
			pp.showdescription(
				`<p class='discription_text'>Click on start button to Start the timer</p>`,
				3
			);
			if (show_once) {
				show_right_pannel(3);
	...
	if (y >= 240 && y <= 290) {
		if (x >= 900 && x <= 970) {
			...
			if (index_temp_con >= 0) {
				...
				pp.showdescription(
					`<p class='discription_text'>Start Timer</p>`,
					3
				);
				console.log(
				...
				pp.showdescription(
					`<p class='discription_text'>stop the time after steady state temp i.e. after 900 sec</p>`,
					3
				);
				show_right_pannel(3);
				...
				var Ti_value = new Chemistry.Geo_Text(
					Ti_text.toString(),
					new Chemistry.Point(310, 324),
					canvas
				);
				...
				var To_value = new Chemistry.Geo_Text(
					To_text.toString(),
					new Chemistry.Point(310, 696),
					canvas
				);
				...
				function anim_timer() {
					...
					if (settime <= 900) {
						...
						pp.showdescription(
							`<p class='discription_text'>stop the timer</p>`,
							3
						);
						canvas.addEventListener(
							'click',
							a6_mouseclick_timer_stop
						);
						show_right_pannel(3);
						anim_timer_900_more();
					}
				...
				function anim_timer_900_more() {
					...
					if (timer_anim) {
						...
						pp.showdescription(
							`<p class='discription_text'>Click to reset the timer</p>`,
							3
						);
						show_right_pannel(3);
...
function a6_mouseclick_timer_reset(e: MouseEvent) {
	...
	if (y >= 240 && y <= 290) {
		if (x >= 970 && x <= 1015) {
			...
			pp.showdescription(
				`<p class='discription_text'>Close outlet glass valve</p>`,
				3
			);
			show_right_pannel(3);
		}
...
function a6_mouseclick_seq_12(e: MouseEvent) {
	...
	if (y >= 477 && y <= 550) {
		if (x >= 593 && x <= 660) {
			...
			let glass = new Chemistry.anim_image(
				seq10_img,
				new Chemistry.Point(470, 440),
				709,
				650,
...
function a6_mouseclick_seq_14(e: MouseEvent) {
	let x = Math.round((e.clientX - rect.x) / lscale);
	let y = Math.round((canvas.height - (e.clientY - rect.y)) / lscale);
	console.log(x, y);
	if (y >= 477 && y <= 550) {
...
function load_observation_table() {
	...
	pp.showdescription(
		`<p class='discription_text'>Fill the observation table and verify the data</p>`,
		3
	);
	show_right_pannel(3);
...
function load_observation_table_data() {
   ...
   <table class="table">
	...
	pp.addtorightpannel(
		`<button id="panel2_btn" class="btn btn-primary" onclick="activity7()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Next</button>`,
		3
	);
	show_right_pannel(3);
}
...
function verify_ob_table() {
	...
	let val1: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('T1-inp')
	);
	let val2: HTMLInputElement = <HTMLInputElement>(
		document.getElementById('T2-inp')
	);
	...
	let btn: HTMLButtonElement = <HTMLButtonElement>(
		document.getElementById('verify-ob-btn')
	);
	btn.disabled = true;
