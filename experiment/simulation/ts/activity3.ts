...
function activity3() {
	...
	pp.showtitle(
		`<p id='exp-title'>Figure shows the experimental setup. The assembled set up will be used to carry out the experimentation to study the heat transfer coefficient, in a shell and tube heat exchanger.</p>`,
		3
	);
	pp.showdescription(
...
function touchmove1(e: TouchEvent) {
	if (geo == 'Drag') {
		...
		let y = Math.round(
			(canvas.height - (e.touches[0].clientY - rect.y)) / lscale
		);
		drag_geo1(x, y);
		// display_labels();
	}
...
function drag_geo1(x: number, y: number) {
	for (let i = 0; i < scene.container.length; i++) {
		if (
			scene.container[i].geo.isinside(new Chemistry.Point(x, y)) &&
			...
			if (i > 5) {
				movable_hint = new Chemistry.Rectangle(
					12,
					30,
					new Chemistry.Point(
						x + adj[i + 1][1].x,
				...
				fixed_hint = new Chemistry.Rectangle(
					12,
					30,
					new Chemistry.Point(adj[i + 1][0].x, adj[i + 1][0].y),
					canvas
				);
				...
				movable_hint = new Chemistry.Rectangle(
					12,
					30,
					new Chemistry.Point(x + adj[i][1].x, y + adj[i][1].y),
					canvas
				);
				fixed_hint = new Chemistry.Rectangle(
					12,
					30,
					new Chemistry.Point(adj[i][0].x, adj[i][0].y),
					canvas
				);
				if (hint_orientation[i] == 'h') {
					movable_hint.angle = 90;
...
function assemble1(obj: Chemistry.Geometry) {
	for (let i = 0; i < scene1.container.length; i++) {
		if (
			scene1.container[i].geo.isinside(
				new Chemistry.Point(obj.stpt.x, obj.stpt.y)
			) &&
...
function a3_display_msg() {
	if (current_msg_i >= a3_msg.length) {
		...
		pp.showdescription(
			`<p class='discription_text' > ${a3_msg[current_msg_i]} </p>`,
			3
		);
	}
}
...
function create_activity_4_button() {
	...
	pp.showtitle(
		`<p id='exp-title'>You've completed the task click on next to start new activity</p>`,
		3
	);
