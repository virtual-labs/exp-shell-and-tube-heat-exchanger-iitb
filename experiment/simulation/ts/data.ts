...
let surface_area = (Math.PI / 4) * id_tube ** 2; //(sq.cm)
let heat_transfer_area =
	Math.PI * (id_tube / 100) * (length_tube / 100) * num_tubes; //(sq.m)
let shell_id = 5; //(cm)
let pitch_tri = 0.8; //(cm)
let baf_sp = 5; //(cm)
let C = 0.5; //(cm)
let de =
	(4 * (pitch_tri ** 2 - (Math.PI * od_tube ** 2) / 4)) / (Math.PI * od_tube); //(cm)
...
function add_std_deviation() {
	for (let i = 0; i < shell_table.t.length; i++) {
		shell_table.t[i] = parseFloat(
			std_deviation(shell_table.t[i]).toFixed(4)
		);
		shell_table.T1[i] = parseFloat(
			std_deviation(shell_table.T1[i]).toFixed(4)
		);
		shell_table.T2[i] = parseFloat(
			std_deviation(shell_table.T2[i]).toFixed(4)
		);
	}
}
...
for (let i = 0; i < shell_table.t.length; i++) {
	...
	shell_table['m453'][i] = shell_table['m'][i] ** 0.453;
	shell_table['m1'][i] = 1 / shell_table['m453'][i];
	shell_table['Re'][i] =
		(id_tube * shell_table['u'][i] * rho) / (mue * 10 ** -2);
	...
	shell_table['hi_theo'][i] =
		3.048129612 *
		shell_table['Re'][i] ** 0.453 *
		shell_table['Pr'][i] ** 0.5710080628;
}
