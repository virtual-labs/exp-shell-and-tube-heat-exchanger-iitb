...
var a2_msg = [
    ...
    'Drag the heat exchanger to the assembly area',
...
var fixed_hint;
var movable_hint;
var act2_btn = document.createElement('div');
act2_btn.innerHTML = `<button id="panel2_btn" class="btn btn-primary" onclick="a2_first()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Next</button>`;
function activity2() {
    pp.clearleftpannel();
    pp.addcanvas('mycanvas');
    pp.showtitle(`Observe the assembly.
    Next task is to <span class="text-color-blue">assemble it yourself.<span>
    `, 3);
    pp.showdescription('<p style="background-color: #f4ccccff; border-radius: 10px; border: black; padding: 5%; font-weight: 500; font-size: 1.2vw;">Figure shows the experimental setup. The assembled set up will be used to carry out the experimentation to study the heat transfer coefficient, in a double-pipe heat exchanger with, parallel configuration under the Laminar Flow. <br><br> Click on next when you are ready.</p>', 3);
    canvas = pp.canvas;
    context = canvas.getContext('2d');
    canvas.style.cursor = 'crosshair';
    rect = canvas.getBoundingClientRect();
    scene = new Scene();
    pp.addtorightpannel(act2_btn.innerHTML, 3);
...
function a2_first() {
    ...
    two_section();
    pp.addcanvas('mycanvas');
    pp.showtitle(`Drag and assemble the components from the Component Library to the Assembly Area. Click <span class="text-color-blue" >Start</span> to assemble.`, 3);
    pp.showdescription('<p style="background-color: #f4ccccff; border-radius: 10px; border: black; padding: 5%; font-weight: 500; font-size: 1.2vw;">Assemble the experimental setup. The assembled set up will be used to carry out the experimentation to study the heat transfer coefficient, in a double-pipe heat exchanger with, parallel configuration under the Laminar Flow.</p>', 3);
    show_sections = true;
    canvas = pp.canvas;
    context = canvas.getContext('2d');
    canvas.style.cursor = 'crosshair';
    ...
    btn.innerHTML = `<button id="panel2_btn" class="btn btn-primary" onclick="a2_second()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Start</button>`;
...
function a2_second() {
    ...
    pp.showtitle(`Drag and assemble the components from the Component Library to the Assembly Area. Click <span class="text-color-blue" >Start</span> to assemble.`, 3);
    pp.showdescription('<p style="background-color: #f4ccccff; border-radius: 10px; border: black; padding: 5%; font-weight: 500; font-size: 1.2vw;">Assemble the experimental setup. The assembled set up will be used to carry out the experimentation to study the heat transfer coefficient, in a double-pipe heat exchanger with, parallel configuration under the Laminar Flow.</p>', 3);
    // document.getElementById('a2-btn').remove();
...
function a2_windowresize() {
    ...
    // let show_score = new Chemistry.Text(`Score: ${a2_total_score}/11`, new Chemistry.Point(50, 650), canvas);
    ...
    for (let i = 0; i < scene1.container.length; i++) {
        if (scene1.container[i].geo.isinside(new Chemistry.Point(obj.stpt.x, obj.stpt.y)) &&
            ...
            if (obj.name == obj_names[current_msg_i]) {
                a2_total_score++;
                global_score++;
                //document.getElementById("a2-score-div-box").innerText = `${global_score}`;
            }
...
function a2_display_msg() {
    if (current_msg_i >= a2_msg.length) {
        ...
        pp.showtitle(a2_msg[current_msg_i], 3);
    }
}
...
function a2_remove_event() {
    ...
    canvas.addEventListener('mouseup', mouseup1);
    canvas.addEventListener('touchmove', touchmove1);
    window.removeEventListener('load', a2_windowresize);
    window.removeEventListener('resize', a2_windowresize);
}
function create_activity_6_button() {
    console.log('created');
    pp.showtitle(`You've completed the task click on next to go to next task`, 3);
    pp.showdescription(``, 3);
    var act2_btn = document.createElement('div');
    act2_btn.innerHTML = `<button id="panel2_btn" class="btn btn-primary" onclick="activity6()" style="position: absolute; font-size:1.1vw; left:2.5vh; bottom: 12vh;  width: 90%;">Next</button>`;
    pp.addtorightpannel(act2_btn.innerHTML, 3);
    show_right_pannel(3);
}
// activity2();
